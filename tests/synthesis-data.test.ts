...
test('synthesis 带 tool_outputs 时,report 注入 tool_result 结论', async () => {
  ...
  const r = await llm.generateStructured<{ findings: Array<{ id: string; statement: string; source: string }>; sub_questions: Array<{ finding_ids: string[] }> }>({
    prompt: '基于真实执行结果生成报告',
    schema: {},
  ...
  // 注入的检索发现必须被某个子问题引用,兑现"报告消费了真实检索数据"的链路证明。
  const injected = r.data.findings.find((f) => f.statement.includes('淘宝'));
  assert.ok(injected?.id, '注入结论应带 id');
  const referenced = (r.data.sub_questions ?? []).some((sq) => (sq.finding_ids ?? []).includes(injected!.id));
  assert.ok(referenced, '注入的检索发现应被某个子问题引用');
});
