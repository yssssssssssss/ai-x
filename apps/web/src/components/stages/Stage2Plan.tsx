...
import type { PlanResponse, PlanStep } from '../../api/client.ts';
import { Header } from './Stage1Understand.tsx';

// 段2 · 待执行计划(HITL 硬闸门):步骤列表(SKILL/TOOL 徽章)+ 假设可就地编辑 + 确认按钮。
// locked=true 时(已进入执行)隐藏确认按钮、禁用编辑。
export function Stage2Plan({
  plan, locked, onConfirm,
}: {
  plan: PlanResponse; locked: boolean; onConfirm: () => void;
}) {
    ...
    <section style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 18, marginBottom: 16 }}>
      ...
      {!locked && !confirmed && (
        <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
          <button className="btn-primary" onClick={() => { setConfirmed(true); onConfirm(); }}>
            ✓ 确认计划,开始执行
          </button>
        </div>
      )}
