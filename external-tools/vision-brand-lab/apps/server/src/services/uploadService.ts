...
export const saveUpload = async (file: MultipartFile) => {
  ...
  return { id: fileName, fileName: file.filename, path, mimeType: file.mimetype };
};

export const assertInsideUploadDir = (path: string): string => {
  const resolved = resolve(path);
  const uploadRoot = resolve(env.uploadDir);
  if (!resolved.startsWith(uploadRoot)) throw new Error('file path must be inside upload directory');
  return resolved;
};
