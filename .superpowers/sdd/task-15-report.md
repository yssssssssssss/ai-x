...
- Same-path retry is serialized in PostgreSQL with a transaction-scoped advisory lock derived from exact `storage_uri` at both STAGING creation and seal. A live STAGING/SEALED owner blocks creation/seal, preventing concurrent double-SEALED rows.
...
- Cleanup/reuse RED: successful temp retention, failed publication retention, same-bytes retry, different-bytes rejection, STAGING rejection, and reconcile no-move were exercised. GREEN: binary 32/32, ControlPlane 38/38; exact combined 70/70 and `pnpm typecheck` passed.
