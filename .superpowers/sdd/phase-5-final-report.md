...
3. Composer validates annotation `derivedFrom` against the exact verified original and emits an original-first `image-comparison` block without a duplicate standalone annotation block.
...
- Main observed the six-file Phase 5 integrated suite at 137 total / 136 pass / 1 existing provider skip / 0 fail.
- After extracting the pure ReportDocument view-model from the JSX module without behavior or assertion changes, Main observed the focused report-document + report-bundle suite at 32/32, `pnpm typecheck` passed, and the Web production build passed.
