...
- Deterministic requirement/question/evidence/Finding Graph/recommendation-root gates run before reviewer LLM; deterministic failures block and pause without an LLM call.
...
- Integrated optional review into `LeaseExecutionEngine` with `executing -> reviewing -> composing_report -> completed` transitions and result propagation through workflow/API contracts. Existing CurrentDeliverable v1 envelope remains unchanged.
...
- Command-loss replay now verifies the SEALED final Review, reuses the same dimension invariant, follows its exact `deliverableArtifactId`, verifies the Evidence Manifest, and reconstructs all terminal IDs without rerunning the execution driver.
